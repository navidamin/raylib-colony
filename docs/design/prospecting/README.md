...
| 13 | [prototypes/layer-block.html](prototypes/layer-block.html) | **LAYER BLOCK.** The four exploded plates rebuilt as one solid body cut into four strata, with the drill bar unchanged beside it. **6 km of ground centred on a sect, 2 km deep**, pocked with a few small scattered craters. Real LOLA ground is available but **off by default** — the Real ground card reads the block's large-scale shape from the LDEM_16 the game ships (1.9 km/px, so ~3 samples across the block: elevation, regional tilt and broad form measured, finer synthetic) with a hillshaded locator of the real 182 km around Tycho or Mare Serenitatis, with that sect drawn as a dome at its real 1 km across (the branch's `terrain_synthesis.h` still uses `coreRadiusKm = 1.55`, a 3.1 km dome — the prototype follows the 1 km design decision and the game's terrain constants are untouched); built ground cannot be drilled, and the vertical exaggeration is computed and printed. Click a layer and everything above it comes off, leaving it translucent. A **shape bench**: the interfaces are generated, not measured, and ~30 levers drive relief, conformity, dip and dip fan, a buried impact bowl, stratum thickness, tilt, lattice, light and texture, plus **fog of war**: under fog the ground has no type and no boundaries at all (an unknown body under the real strata), and fog is exactly the ground that has not collapsed. What collapses it is **drilling**, in two decisions: arm the rig from its dock and click the ground to **spud in** (the bit sinks its own length, the handle lands on the ground), then the **borehole bar flashes and becomes the control** — click a depth on it and the string is fed down to exactly there — the handle stays on the ground and the shaft runs through it, so the tail above it is visibly used up — scouring a bowl with a rim of spoil and throwing ejecta the colour of the bed it is cutting. It trips out leaving a turning wireframe core barrel in the hole. Every hole improves the **whole model**, not a stripe of it — a great deal where it stands, a little from the far corner, nothing below the depth it reached — and what the fog hides is an **estimate**, so each hole visibly re-fits it: beds rise, dips shallow out, a level bed turns out to roll. At 5% fog the model is called settled and stops moving. A **delineation** bar reads mean confidence over the block (INFERRED → INDICATED → MEASURED) and the layer-peeling controls stay locked until it clears 95%, about eight well-spread full-depth holes. **Drag to turn the block**: all four walls are cut and fogged, and every hole speaks to every one of them, most to the wall it stands nearest. "Copy values" lifts the numbers out. Straight shared depth axis, so the bands are true thickness. Graphics per [Dark Plating 9.47–9.49](../graphics/dark-plating.md) | PROTOTYPE — not yet ported |
| 14 | [prototypes/regolith-layer.js](prototypes/regolith-layer.js) | **REGOLITH LAYER, standalone.** One stratum lifted out of #13 with no dependencies: the ported rock texture, its two interfaces, and the code that draws them, with every lever that touches it documented in one `P` object. [regolith-demo.html](prototypes/regolith-demo.html) renders nine lever settings side by side | PROTOTYPE |
| 15 | [survey-dashboard-design.md](survey-dashboard-design.md) | **THE NEW LOOK — all decisions settled.** The module as a five-panel console on a 1536x1024 field: a machined tool rack, a live *software-3D* holographic block with yaw, pitch, explode and isolate, a scrolling survey log, the drill bar and rig telemetry, in navy and cyan instead of near-black and amber. Supersedes the panel chrome, layout and interaction of #9 and #13 and **nothing about the mechanics** -- it carries the map from every existing mechanic to its new home, the four things the change costs, and seven decisions still open. The drill (glyph, cursor, planted rig, bar rig) is explicitly out of scope and moves across unchanged, keeping its own design inside the new one. **Settled:** five tools in five bays; fog becomes unresolved wireframe (the console draws only what it has measured); **isolate only, peel is cut**; on a phone the block is the app and the rack, drill bar and log are summoned; telemetry derived, never invented; the 37-lever bench behind `?bench=1`; and the look is scoped to **prospecting and excavation only** | DESIGN -- plan of record for presentation |
| 16 | [survey-dashboard-implementation.md](survey-dashboard-implementation.md) | **THE BUILD PLAN** for #15. Ten stages, each independently rendered and verified, ordered so the riskiest (the drill under a pitched camera) is proved second rather than last; the three hard parts worked through; a harness table, a risk register, and what is deliberately not in it | PLAN -- **the game port is under way**: C1-C6 done and C7 all but the retirement; the console is now `src/ui/` (c2d shim, Holo3D, ToolRack, dash_chrome, survey_dash). See its section 7 |
| 17 | [prototypes/dashboard/](prototypes/dashboard/) | The three source files for #15, vendored verbatim: [`dashboard.html`](prototypes/dashboard/dashboard.html) (the whole console -- `ToolRack`, `HoloBlock`, `Holo3D`, `Dashboard`, and the `DASH` config), [`holo3d.js`](prototypes/dashboard/holo3d.js) (the block renderer alone), [`layers-block-3d.html`](prototypes/dashboard/layers-block-3d.html) (that renderer with a bare driver), and [`dashboard.png`](prototypes/dashboard/dashboard.png), the reference render. **Rev. 2026-09-11:** the scan wavefront removed, and bed isolation fixed by compositing ghost groups through offscreen buffers so stacked ghosts stop accumulating | SOURCE |
| 18 | [prototypes/survey-dashboard.html](prototypes/survey-dashboard.html) | **THE CONSOLE, being built** to #15 and #16. **Stage 0 done:** the ground, the horizontal module bar, five bracketed panels, the two branch connectors and the titles -- at **both breakpoints**, with the rack and the drill bar opening and closing on both. Every rectangle comes out of one `Layout(containerWidth)` function and no geometry is a constant, because the phone is a different arrangement rather than a narrower one. On a phone the block owns the first screen, the log is below the fold, and a two-button strip below the confidence bar summons the rack (from the left) and the drill bar (from the top) -- drawn last, so a sheet can never bury its own toggle. Empty panels name the stage that will fill them. **Stage 1 done:** the block is ours -- `BuildHeights` ported from #13, four beds on a 28x28 lattice with the craters, dip and conformity it has always had, fed into Holo3D's camera in place of its six measured profiles, rotatable in yaw and pitch. **Stage 2 done:** the drill, unchanged, on a camera that pitches -- arm, spud, set depth, cut, trip out, bore marker, spatter and scour, at every yaw and pitch, because world-vertical projects to screen-vertical and the rig never needed redrawing. Placing a hole is a ray march onto the cap | STAGES 0-3 DONE -- and now a BENCH, not the product: the module ships as raylib C++ in `src/Survey/`, and the prototype's remaining stages are built directly in the game. **Stage 3 done:** fog re-expressed -- the console draws only what it has measured, unknown volume gets no fill and no boundary, and what is left is a wire cage that retreats as the block is drilled |
| 19 | [console-real-data.md](console-real-data.md) | **THE CONSOLE ON REAL DATA.** The seven-phase plan that replaces the ported console's demo content with the game's own: one state object, owned by the module; one knowledge model; one 120 m column; then the rack, ruler and block fed from `ProspectingSystem`, and finally a drilled hole that moves `scanMultiplier`. Carries the C/C++ boundary, what each phase measured, and the two parts of #15 it defers with reasons | P1-P5 BUILT, P6-P7 PLANNED |
...
> **Three designs live here, and they answer different questions.**
> #1-#8 describe **v1** as it was built: the panel had tabs, a lab and GPR,
> and depth was tier-gated. All of that is gone -- read them for instrument
> detail only.
> **#9 is the design of record for the loop** (drill holes, an interpolated
> estimate, a four-layer block) and **#10 for progression** (rigs you buy,
> techniques you learn).
> **#15 is the design of record for the look** -- the five-panel console --
> and it supersedes the chrome, layout and interaction of #9 and #13 while
> changing nothing about what any of them *do*. #16 is its build plan.
> Start at #9, #10 and #15 for new work.
