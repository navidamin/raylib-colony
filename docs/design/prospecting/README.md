...
| 13 | [prototypes/layer-block.html](prototypes/layer-block.html) | **LAYER BLOCK.** The four exploded plates rebuilt as one solid body cut into four strata, with the drill bar unchanged beside it. Click a layer and everything above it comes off: the layer is left translucent, so its surface and the floor under it read at once -- shape on the top, class map on the floor. Straight shared depth axis, so the bands are finally true thickness. Graphics per [Dark Plating 9.47](../graphics/dark-plating.md) | PROTOTYPE -- not yet ported |
