...
| 16 | [survey-dashboard-implementation.md](survey-dashboard-implementation.md) | **THE BUILD PLAN** for #15. Ten stages, each independently rendered and verified, ordered so the riskiest (the drill under a pitched camera) is proved second rather than last; the three hard parts worked through; a harness table, a risk register, and what is deliberately not in it | PLAN -- **the game port is under way**: C1 done (`src/Survey/`, the block in `DrawProspectingPanel`), C2-C7 to come. See its section 7 |
