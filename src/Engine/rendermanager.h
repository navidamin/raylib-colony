...
private:
    ...
    //
    // One chain per place gives the views their ground: step 0 (100 km,
    // once the retired Planet view's), 1 = COLONY (25 km), 2 = SECT (5 km),
    // each the centre crop of the one above and all registered on the
    // same LunarPoint, so zooming is continuous. (The chain's steps, not
    // levels: the level ladder is Globe -> District -> Site.) The colony you are
    ...
    // 33-frame hitch. Two ways to build one, chosen by
    // TerrainChainOnGpu(): the CPU chain (~0.5 s, so it runs on worker
    // threads -- or on demand, in a browser) or the GPU chain
    // (milliseconds, main thread, one per frame). Nine chains at 512 is about 28 MB; at the GPU's 1024,
    // 113 MB.
