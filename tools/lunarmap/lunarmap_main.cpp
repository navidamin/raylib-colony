...
static int RenderLadder(AppState& app)
{
    ...
    SurveyViewport viewport = SurveyLadderViewport(opts.width, opts.height);
    ...
    if (demo)
    {
        ...
        if (BuildScene(l1, *app.dem, app.scene))
        {
            ...
            for (int pass = 0; pass < passes; pass++)
            {
                ...
                int hover = LunarRegionAt(hLat, hLon);
                bool hoverPkt = InProcellarumKreepTerrane(hLat, hLon);
                ...
                if (alt)
                {
                    const char* hn = (hover >= 0)
                        ? GetLunarRegions()[(size_t)hover].name.c_str()
    ...
    for (;;)
    {
        ...
        if (level == SITE_LEVELS - 1)
        {
            double cLat = 0.0, cLon = 0.0;
            SurveyCursorLatLon(*cursor, &cLat, &cLon);
            siteB = app.dem->EvaluateSite(cLat, cLon, cursor->footprintKm, SITE_VERDICT_HORIZON_KM);
